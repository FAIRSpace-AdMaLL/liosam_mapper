...
mapOptimization::mapOptimization()
{
    ...
    pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/cloud_registered_raw", 1);  // rviz visualization
...
{
    if (saveFrame() == false || laserCloudCornerLastDS->size() == 0 || laserCloudSurfLastDS->size()== 0)
        return;
